...
<style>
  ...
  .dossier-numero { font-family:'Playfair Display',serif; color:#fff; font-size:1.5rem; margin:0 0 .3rem; }
  .dossier-meta   { color:rgba(255,255,255,.55); font-size:.84rem; display:flex; flex-wrap:wrap; gap:.4rem 1.2rem; }
  ...
  .eleve-photo-wrap img { width:100%; height:100%; object-fit:cover; }

  .section-card { background:#fff; border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 4px 20px rgba(13,27,42,.07); margin-bottom:1.2rem; overflow:hidden; }
  .section-card-head { padding:.9rem 1.4rem; border-bottom:1px solid #E2E8F0; display:flex; align-items:center; gap:.6rem; background:#FAFBFC; }
  .section-card-head h5 { font-family:'Playfair Display',serif; font-size:.95rem; color:#0D1B2A; margin:0; }
  .section-card-head i  { color:#2563EB; font-size:1rem; }
  .section-card-body { padding:1.2rem 1.4rem; }

  .info-grid { display:grid; grid-template-columns:1fr 1fr; gap:.5rem 2rem; }
  .info-item { padding:.45rem 0; border-bottom:1px solid #F1F5F9; }
  .info-item:last-child { border:none; }
  .info-item .lbl { color:#64748B; font-size:.75rem; font-weight:600; text-transform:uppercase; letter-spacing:.04em; margin-bottom:.18rem; }
  ...
  .tl-step { flex:1; text-align:center; position:relative; padding-top:2.5rem; }
  .tl-step::before { content:''; position:absolute; top:15px; left:calc(-50% + 15px); right:calc(50% + 15px); height:3px; background:#E2E8F0; }
  .tl-step:first-child::before { display:none; }
  .tl-step.done::before   { background:#10B981; }
  .tl-step.active::before { background:linear-gradient(90deg,#10B981,#2563EB); }
  .tl-dot { width:30px; height:30px; border-radius:50%; position:absolute; top:0; left:50%; transform:translateX(-50%); display:flex; align-items:center; justify-content:center; font-size:.8rem; z-index:1; }
  .tl-step.done   .tl-dot { background:#10B981; color:#fff; }
  .tl-step.active .tl-dot { background:#2563EB; color:#fff; box-shadow:0 0 0 4px rgba(37,99,235,.15); }
  .tl-step.pending .tl-dot{ background:#E2E8F0; color:#94A3B8; }
  .tl-label { font-size:.76rem; font-weight:600; color:#94A3B8; margin-top:.2rem; }
  .tl-step.done   .tl-label { color:#10B981; }
  .tl-step.active .tl-label { color:#2563EB; }
  .tl-date  { font-size:.7rem; color:#CBD5E1; }

  /* Documents */
  .doc-item { display:flex; align-items:center; gap:.9rem; padding:.7rem 0; border-bottom:1px solid #F1F5F9; }
  .doc-item:last-child { border:none; }
  .doc-ico { width:38px; height:38px; border-radius:9px; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:1.05rem; }
  .ico-ok   { background:rgba(16,185,129,.1); color:#10B981; }
  .ico-miss { background:rgba(100,116,139,.1); color:#94A3B8; }
  .doc-name { font-size:.85rem; font-weight:600; color:#0D1B2A; }
  .doc-sub  { font-size:.75rem; color:#94A3B8; }
  .badge-req { background:rgba(244,63,94,.08); color:#9F1239; border:1px solid rgba(244,63,94,.2); border-radius:20px; padding:.1rem .55rem; font-size:.7rem; font-weight:700; }
  .badge-opt { background:#F1F5F9; color:#64748B; border:1px solid #E2E8F0; border-radius:20px; padding:.1rem .55rem; font-size:.7rem; font-weight:700; }

  .actions-bar { display:flex; gap:.7rem; flex-wrap:wrap; padding:1rem 1.4rem; background:#FAFBFC; border-top:1px solid #E2E8F0; }
  ...
  .motif-refus strong { color:#9F1239; font-size:.88rem; }
  .motif-refus p { color:#7F1D1D; font-size:.85rem; margin:.3rem 0 0; }
</style>
@endpush
...
<div class="section-card anim-up">
  ...
  <div class="section-card-body">
    <ul class="timeline">
      ...
      <li class="tl-step {{ $inscription->statut === 'validee' ? 'done' : ($inscription->statut === 'refusee' ? 'done' : 'pending') }}">
        <div class="tl-dot">
          @if($inscription->statut === 'validee')
            <i class="bi bi-check-lg"></i>
      ...
      <li class="tl-step {{ $inscription->statut === 'validee' && $inscription->classe ? 'done' : ($inscription->statut === 'validee' ? 'active' : 'pending') }}">
        <div class="tl-dot">
      ...
      <div class="section-card-body">
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <div class="val">{{ \Carbon\Carbon::parse($inscription->eleve->date_naissance)->isoFormat('D MMMM YYYY') }}</div>
          </div>
          <div class="info-item">
      ...
      <div class="section-card-body">
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <div class="val">{{ $inscription->eleve->parent->adresse }}, {{ $inscription->eleve->parent->ville }}</div>
          </div>
    ...
    <div class="section-card anim-up">
      ...
      <div class="section-card-body">
        ...
        <div class="info-item">
          ...
          <div class="val">{{ $inscription->type === 'nouvelle' ? 'Nouvelle inscription' : 'Réinscription' }}</div>
        </div>
        <div class="info-item">
    ...
    @if($eleve->inscription->classe_id)
      <a href="{{ route('parent.inscription.carte', $eleve->inscription->id) }}"
        ...
        <a href="{{ route('parent.dashboard') }}"
           class="btn-outline-gu ms-auto" style="font-size:.83rem;padding:.48rem 1rem">
          ...
          <div class="doc-item">
            ...
            @if($doc)
              <a href="{{ $doc->url }}" target="_blank"
                 class="btn-outline-gu" style="padding:.28rem .65rem;font-size:.75rem">
                <i class="bi bi-eye"></i>
              </a>
            @else
