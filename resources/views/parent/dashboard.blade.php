...
@section('page_subtitle', 'Bienvenue, ' . ($parent->prenom ?? 'Parent') . ' !')
...
<style>
  ...
  .welcome-banner::after {
    ...
    background:rgba(56,189,248,.07); pointer-events:none;
  }
  .welcome-banner h2 { font-family:'Playfair Display',serif; color:#fff; font-size:1.45rem; margin:0 0 .3rem; }
  .welcome-banner p  { color:rgba(255,255,255,.6); font-size:.88rem; margin:0; }
  .welcome-date { position:absolute; right:2.2rem; top:50%; transform:translateY(-50%); text-align:right; }
  .welcome-date .day  { font-family:'Playfair Display',serif; font-size:2.8rem; color:#fff; line-height:1; }
  .welcome-date .mois { color:#38BDF8; font-size:.8rem; font-weight:600; text-transform:uppercase; letter-spacing:.06em; }
  ...
  .stat-icon  { width:50px; height:50px; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:1.35rem; flex-shrink:0; }
  .stat-val   { font-family:'Playfair Display',serif; font-size:1.85rem; color:#0D1B2A; line-height:1; }
  .stat-label { color:#64748B; font-size:.8rem; margin-top:.2rem; }

  .eleve-card { background:#fff; border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 4px 24px rgba(13,27,42,.09); overflow:hidden; margin-bottom:1rem; transition:transform .2s; }
  .eleve-card:hover { transform:translateY(-2px); }
  .eleve-head { background:linear-gradient(110deg,#0D1B2A,#1A3A5C); padding:1.1rem 1.4rem; display:flex; align-items:center; gap:.9rem; }
  .eleve-avatar { width:50px; height:50px; border-radius:50%; flex-shrink:0; border:3px solid rgba(255,255,255,.2); background:rgba(255,255,255,.1); display:flex; align-items:center; justify-content:center; font-size:1.3rem; color:#38BDF8; overflow:hidden; }
  .eleve-avatar img { width:100%; height:100%; object-fit:cover; }
  .eleve-nom    { font-family:'Playfair Display',serif; color:#fff; font-size:.97rem; margin:0; }
  .eleve-classe { color:#38BDF8; font-size:.77rem; }
  .eleve-body   { padding:1rem 1.4rem; }
  .eleve-row    { display:flex; justify-content:space-between; align-items:center; padding:.4rem 0; border-bottom:1px solid #E2E8F0; font-size:.83rem; }
  .eleve-row:last-child { border:none; }
  .eleve-row .k { color:#64748B; }
  .eleve-row .v { color:#0D1B2A; font-weight:600; }
  .eleve-foot   { padding:.8rem 1.4rem; background:#FAFBFC; border-top:1px solid #E2E8F0; display:flex; gap:.5rem; flex-wrap:wrap; }

  .echeance-card { background:linear-gradient(115deg,#7C3AED,#5B21B6); border-radius:14px; padding:1.4rem 1.6rem; color:#fff; position:relative; overflow:hidden; margin-bottom:1.5rem; }
  .echeance-card::after { content:''; position:absolute; right:-30px; bottom:-30px; width:130px; height:130px; border-radius:50%; background:rgba(255,255,255,.05); pointer-events:none; }
  .echeance-card h6  { color:rgba(255,255,255,.6); font-size:.72rem; font-weight:600; text-transform:uppercase; letter-spacing:.08em; margin-bottom:.4rem; }
  .echeance-amount   { font-family:'Playfair Display',serif; font-size:1.65rem; margin:.2rem 0; }
  .echeance-sublabel { color:rgba(255,255,255,.65); font-size:.82rem; }
  .echeance-date     { display:inline-flex; align-items:center; gap:.4rem; background:rgba(255,255,255,.15); border-radius:20px; padding:.28rem .75rem; font-size:.77rem; margin-top:.65rem; }

  .action-grid { display:grid; grid-template-columns:1fr 1fr; gap:.75rem; margin-bottom:1.5rem; }
  .action-btn  { background:#fff; border:1.5px solid #E2E8F0; border-radius:11px; padding:1rem; display:flex; flex-direction:column; align-items:center; gap:.5rem; text-decoration:none; transition:all .2s; text-align:center; }
  .action-btn:hover { border-color:#2563EB; background:#EFF6FF; transform:translateY(-2px); }
  .action-btn i    { font-size:1.45rem; background:linear-gradient(135deg,#2563EB,#38BDF8); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
  .action-btn span { color:#0D1B2A; font-size:.81rem; font-weight:600; }
  .action-btn.disabled { opacity:.4; pointer-events:none; }

  .notif-list { border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 4px 24px rgba(13,27,42,.09); overflow:hidden; background:#fff; }
  .notif-item { display:flex; align-items:flex-start; gap:.9rem; padding:.95rem 1.3rem; border-bottom:1px solid #E2E8F0; transition:background .15s; cursor:pointer; }
  .notif-item:last-child { border:none; }
  .notif-item:hover  { background:#FAFBFD; }
  .notif-item.unread { background:#EFF6FF; }
  .notif-icon  { width:40px; height:40px; border-radius:10px; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:1.05rem; }
  .ni-pay      { background:rgba(245,158,11,.12); color:#F59E0B; }
  .ni-info     { background:rgba(37,99,235,.1);   color:#2563EB; }
  .ni-alert    { background:rgba(244,63,94,.1);   color:#F43F5E; }
  .notif-title { font-size:.87rem; font-weight:600; color:#0D1B2A; margin:0 0 .18rem; }
  .notif-body  { font-size:.8rem;  color:#64748B; margin:0 0 .25rem; }
  .notif-time  { font-size:.74rem; color:#94A3B8; }
  .unread-dot  { width:8px; height:8px; border-radius:50%; background:#2563EB; flex-shrink:0; margin-top:.4rem; }

  @media(max-width:767px){ .welcome-date{ display:none; } }
</style>
@endpush
...
<div class="row g-4">
  ...
  <div class="col-lg-7">
    <div class="section-head">
      ...
      <a href="{{ route('parent.inscription.create') }}" class="link-sm">
      ...
      <div class="eleve-card anim-up">
        ...
        <div class="eleve-body">
          <div class="eleve-row">
            ...
            <span class="v">{{ \Carbon\Carbon::parse($eleve->date_naissance)->isoFormat('D MMMM YYYY') }}</span>
          </div>
          <div class="eleve-row">
            <span class="k"><i class="bi bi-layers me-1"></i>Niveau</span>
            <span class="v">{{ $eleve->inscription?->classe?->niveau ?? $eleve->niveau_souhaite ?? '—' }}</span>
          </div>
          <div class="eleve-row">
          ...
          <div class="eleve-row">
            ...
            <span class="v">{{ $eleve->inscription?->created_at?->isoFormat('D MMM YYYY') ?? '—' }}</span>
          </div>
        </div>
  ...
  <div class="col-lg-5">
    ...
    <div class="action-grid">
      <a href="{{ route('parent.inscription.create') }}" class="action-btn">
        <i class="bi bi-pencil-square"></i>
        <span>Nouvelle inscription</span>
      </a>
      <a href="{{ route('parent.notifications.index') }}" class="action-btn">
        <i class="bi bi-bell"></i>
        <span>Notifications</span>
      </a>
      <a href="#" class="action-btn {{ !$hasValidated ? 'disabled' : '' }}">
        <i class="bi bi-file-earmark-pdf"></i>
        <span>Fiches PDF</span>
      </a>
      <a href="#" class="action-btn {{ !$hasValidated ? 'disabled' : '' }}">
        <i class="bi bi-credit-card-2-front"></i>
        <span>Cartes scolarité</span>
      </a>
    </div>
    ...
    <div class="notif-list">
      @forelse($notifications as $notif)
        <div class="notif-item {{ is_null($notif->lu_le) ? 'unread' : '' }}"
             data-id="{{ $notif->id }}">
